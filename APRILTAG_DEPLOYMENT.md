...
1. **确认你这台 G1 有没有自带可用的 RGB 相机**（EDU 配置一般有 Intel RealSense D435/D435i，
   ...
   支架，外参标定一次就要保持稳定）。
2. **打印 AprilTag**。本仓库用的是 `tag36h11` family（`AprilRobotics/apriltag-imgs` 里的
   `tag36h11/tag36_11_XXXXX.png`），打印时**量清楚黑边方框的实际边长**（不是整张纸的边长，
   是最外圈黑色方框的外边缘到外边缘），这个数字就是后面要用的 `tag_size_m`，必须和贴出来的
   实物一致，差一点距离估计就整体偏一个比例（sim 里踩过这个坑，第 4 节没写但代码注释里有：
   `TAG_SIZE_M = 0.15 * 256/480`，这是渲染贴图的像素换算，真实打印直接拿卡尺量黑框边长）。
3. **在真实箱子上贴 tag**，建议贴 2~3 个面（比如正面 + 顶面）——原因见 5 节，单面在近距离
   会失去视野，多面至少能保证一部分时间可见；另外抓取时手也可能挡住某一面。
