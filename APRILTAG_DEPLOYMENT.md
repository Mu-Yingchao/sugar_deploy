...
### 2.1 ⚠️ 机体系方案的一个陷阱：搬运目标必须每步重算，不能当常量

**（2026-09-23 补充，这是部署机在阶段 4 规划时发现的一个真实设计缺陷，上面的推荐方案照抄
会踩）**：`use_target=True` 的任务（CarryBox/PushBox）除了物体位姿，还要给
`target_obj_pos_b`（搬到哪里去）。上面"anchor 恒等于原点"的简化方案对**物体**是对的——因为
物体每一帧都是通过相机重新观测的，机体系下的物体位置天然是当前时刻的正确值。

**但对目标不成立**。目标是"世界里的一个固定地点"，在机体系下的坐标**必须随机器人走动而
变化**：机器人朝目标走近 1m，下一帧目标在机体系下就应该近 1m。如果按"启动时算一次、之后当
常量传"来实现，目标就会**牢牢跟着机器人一起移动**，机器人永远走不到，表现为一直朝前走却
不收敛——而且这个错误不会报错、数值看起来也完全正常，很难从日志上发现。

正确做法有两种，**都需要知道机器人相对起点走了多远**，也就是说这一项绕不开某种位置估计：

1. **用机器人自身的里程计/状态估计**（如果可靠）：退回上面说的第二种方案，anchor 用里程计
   位姿，目标用世界系常量，每步做一次 `subtract_frame_transform`。
2. **把目标定义成"相对物体"而不是"相对世界"**：比如"把箱子搬到它现在位置往 X 方向 1m 处"，
   由于物体每帧都重新观测，可以每帧基于**当前观测到的物体位姿**重算目标。但要注意语义变了——
   箱子被抱起来跟着机器人走之后，这个定义下的目标也会跟着动，同样到不了。只有在目标可以用
   场景里另一个**能被持续观测到的静态参照物**（比如在终点地面上再贴一个 AprilTag）来定义时，
   这条路才干净——**这其实是最贴合本方案的做法：终点也贴一个 tag，目标位姿直接观测出来，
   不需要任何里程计**。

推荐第 2 种的"终点贴 tag"变体：和物体走同一套感知链路、不引入里程计漂移、标定方法完全一样
（多一个 `TagObjectOffset` 而已）。代价是终点 tag 也要在相机视野里——这和第 5 节说的近场
盲区是同一类约束，需要现场验证覆盖范围。
