...
### ⚠️ 重要更正（2026-09-23 真机实测）：遥控器官方模式键**不能**覆盖我们发的低层指令

**这份文档之前在这里写错了，而且错得危险**——原来写的是"遥控器是独立于我们代码之外的硬件级
急停，不管我们的脚本怎么样这个通道都不受影响，是真正意义上的最后一道防线"。**真机实测证伪了
这个说法**：在我们持续发布 `rt/lowcmd` 低层指令期间，现场多次出现按遥控器官方模式键后机器人
**没有反应**（没有卸力、没有语音播报、仍有较强保持力）的情况。

合理的理解是：`release_high_level_control()` 把官方高层控制释放掉之后，官方模式键走的那条
控制路径也一并失效了，而我们的低层指令仍在以 50Hz 持续覆盖电机——**谁在持续发指令谁说了算**。

**所以正确的认知是**：
- 官方模式键（本机实测：`L2+B`=阻尼、`L2+Y`=零力矩；不同固件版本组合键可能不同，早期文档
  记录 V1.0.2 是 `L1+A`）在**没有低层控制在跑**的时候是有效的（比如还没启动我们的脚本、
  或者脚本已经干净退出之后），这时候它仍然是有用的手段。
- 但**在我们的控制脚本正在发指令期间，不能指望它**。真正兜底的必须是我们自己代码里的停止
  路径：部署机上已经实现的 `SafeRealRobotController`——它直接从 DDS 原始数据里识别手柄组合
  键，锁存停止状态并持续输出零力矩指令，也就是说"急停"这条路径本身是走我们自己的低层通道
  实现的，不是绕过它。
- **这条链路依赖我们的进程、操作系统和 DDS 都还活着**。整机断网、进程被强杀、电脑崩溃这几种
  情况下的行为**没有被验收覆盖**，不能假定安全。

**因此下面这条要求不但没有放松，反而更重要了**：从阶段 1 开始，只要机器人通电且有指令在发，
必须有一个人专职拿着遥控器 + 在机器人旁边做物理保护，不能由写代码/看终端的人兼顾。同时：

- **物理支撑（吊架/坐姿/人扶）不是"保险起见"，是当前唯一不依赖软件的保护**——软件停止链路
  已经被证明有前提条件，物理支撑没有。
- 真正兜底的"硬"手段是**断电**（电源键长按 2 秒以上关机）。**机身上没有查到独立的物理急停
  按钮**，电源键也不是为瞬时急停设计的，但在软件链路失效时它是唯一剩下的手段——事前要知道
  电源键在哪、手能不能立刻够到。
...
不要心存侥幸。结合上面那条实测更正（官方模式键在低层控制期间不可靠），这意味着：

- 部署机上的 `SafeRealRobotController` 已经实现了手柄按键识别、锁存停止、心跳/遥控数据/
  低层状态超时保护，离线单元测试通过，且在 60 秒持续零力矩窗口里现场确认过卸力生效。
  **但它仍然运行在我们自己的进程里**——主循环卡死、进程被强杀、整机断网这几种故障注入
  **没有在真机上验收过**，离线单元测试不能替代现场验收。
- **零力矩指令本身的格式也踩过坑**：原来用的是 `unitree_go` 族的 `PosStopF/VelStopF` 哨兵值，
  真机上发了之后机器人并没有卸力（推测是固件拒收了非法值，于是继续执行上一条有效指令——
  比"卸力失败"更危险，因为程序侧看起来完全正常）。已改为全零格式并在真机确认，详见
  `sugar_deploy/real_robot_io.py` 的 `send_zero_torque()`。
